...
int levenenshteinDistance(std::string s, std::string t)
{
    ...
    for(int j = 1; j < n; ++j)
    {
        for(int i = 1; i < m; ++i)
        {
            ...
            if(s[i - 1] != t[j - 1]) substitutionCost = 1;
            
            d[i][j] = std::min(d[i -1][j] + 1, std::min(d[i][j - 1] + 1,d[i - 1][j - 1] + substitutionCost));            
        }
    }
...
void polymorphic::score::compute(std::string value)
{
	...
	float result = (float)levenenshteinDistance(comp, value);
	if(result > 20.0f) result = 20.0f;
	result = ((result / 20.0f) * -1.0f) + 1.0f;
